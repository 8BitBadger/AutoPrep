...
public class LineOfSight : Area2D
{
    ...
    uint lastBulletFireTime = 0;
    //Turn speed of the turret
    float turnSpeed = Mathf.Deg2Rad(3);//0.1f;
    ...
    private void FaceTarget()
    {
        ...
        {
            if (OS.GetTicksMsec() - lastBulletFireTime > 500)
            {
                ((Gun)GetNode<Node2D>("../Gun/Nozzle")).Fire();
                lastBulletFireTime = OS.GetTicksMsec();
            }
        }
    }
}
